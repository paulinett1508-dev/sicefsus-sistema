...
export const calcularSaldoEmenda = (valorTotal, valorExecutado) => {
  const total = typeof valorTotal === 'number' ? valorTotal : parseValorMonetario(valorTotal);
  const executado = typeof valorExecutado === 'number' ? valorExecutado : parseValorMonetario(valorExecutado);
...
export const calcularEstatisticas = (despesas, emendas) => {
  ...
  const percentualExecutado = totalEmendas > 0 ? (totalDespesas / totalEmendas) * 100 : 0;
