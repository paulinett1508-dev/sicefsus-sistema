...
export const useFormNavigation = (
  ...
  // Navegação para entidade relacionada (ex: emenda -> despesas)
...
/**
 * Hook para navegação entre módulos relacionados (emendas <-> despesas)
 *
 * @param {string} currentModule - Módulo atual ('emendas' ou 'despesas')
...
export const useModuleNavigation = (currentModule, contextData = {}) => {
  ...
  // Navegar para despesas de uma emenda específica
  const goToEmendaDespesas = useCallback(
    (emendaInfo, options = {}) => {
      navigate("/despesas", {
        state: {
  ...
  // Criar despesa para emenda específica
  const createDespesaForEmenda = useCallback(
    (emendaInfo, options = {}) => {
      navigate("/despesas", {
        state: {
          novoDespesa: true,
  ...
  const getNavigationSource = useCallback(() => {
    return {
      ...
      fromDespesas: location.state?.fromDespesas || false,
