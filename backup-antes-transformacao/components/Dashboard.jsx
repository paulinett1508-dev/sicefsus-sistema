...
export default function Dashboard({ usuario }) {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      try {
        const [emendasSnap, lancSnap] = await Promise.all([
          getDocs(collection(db, "emendas")),
          getDocs(
            query(collection(db, "lancamentos"), orderBy("data", "desc")),
          ),
        ]);

        setEmendas(emendasSnap.docs.map((d) => ({ id: d.id, ...d.data() })));
        setLancamentos(lancSnap.docs.map((d) => ({ id: d.id, ...d.data() })));
      } catch (error) {
  ...
  const valorTotal = emendas.reduce((sum, e) => sum + (e.valorTotal || 0), 0);
  ...
  const emendasPorMes = emendas.reduce((acc, e) => {
    if (!e.data) return acc;
    const mes = new Date(e.data).toLocaleString("pt-BR", {
      month: "short",
      year: "numeric",
    });
    acc[mes] = (acc[mes] || 0) + 1;
  ...
  const emendasProximasVencimento = emendas.filter((e) => {
    if (!e.validade || e.saldo <= 0) return false;
      ...
      <div style={styles.welcomeSection}>
        <div>
          ...
          <p style={styles.welcomeSubtitle}>
            Aqui está um resumo da situação das emendas parlamentares
          </p>
        </div>
      ...
      <div style={styles.kpiGrid}>
        ...
        >
          ...
          <div style={styles.kpiContent}>
            ...
            <span style={styles.clickHint}>Clique para gerenciar</span>
          </div>
        </div>
        ...
        >
          ...
          <div style={styles.kpiContent}>
            ...
            <p style={styles.kpiLabel}>Despesas</p>
            <span style={styles.clickHint}>Ver detalhes</span>
          </div>
        </div>
        ...
        <div style={{ ...styles.kpiCard, borderLeft: `4px solid ${WARNING}` }}>
          ...
          <div style={styles.kpiContent}>
            ...
            <p style={styles.kpiLabel}>Valor Total</p>
            <span style={styles.kpiSubtext}>Emendas cadastradas</span>
          </div>
        </div>
        ...
        <div style={{ ...styles.kpiCard, borderLeft: `4px solid ${ERROR}` }}>
          ...
          <div style={styles.kpiContent}>
            ...
            <span style={styles.kpiSubtext}>
              {percentualExecutado.toFixed(1)}% do total
            </span>
          </div>
        </div>

        <div style={{ ...styles.kpiCard, borderLeft: `4px solid ${SUCCESS}` }}>
          <div style={styles.kpiIcon}>💳</div>
          <div style={styles.kpiContent}>
            <h3 style={{ ...styles.kpiValue, color: SUCCESS }}>
              {formatCurrency(saldoDisponivel)}
            </h3>
            <p style={styles.kpiLabel}>Saldo Disponível</p>
            <span style={styles.kpiSubtext}>Pronto para execução</span>
          </div>
        </div>
        ...
        <div
          style={{
            ...styles.kpiCard,
            ...(activeCard === "relatorios" ? styles.activeCard : {}),
            borderLeft: `4px solid ${PRIMARY}`,
          }}
          onClick={() => handleCardClick("relatorios", "/relatorios")}
        >
          <div style={styles.kpiIcon}>📈</div>
          <div style={styles.kpiContent}>
            ...
            <p style={styles.kpiLabel}>Execução Geral</p>
            <span style={styles.clickHint}>Ver relatórios</span>
          </div>
        </div>
      </div>

      {/* Alertas importantes */}
      {(emendasVencidas.length > 0 || emendasProximasVencimento.length > 0) && (
