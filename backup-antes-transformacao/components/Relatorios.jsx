...
export default function Relatorios({ usuario }) {
  ...
  const getOverviewData = () => {
    ...
    return {
      ...
      ticketMedio:
        lancamentos.length > 0 ? valorExecutado / lancamentos.length : 0,
    };
  };
  ...
  const getExecucaoPorAutor = () => {
    ...
    emendas.forEach((emenda) => {
      ...
      const lancamentosAutor = lancamentos.filter(
        (l) => l.emendaId === emenda.id,
      );
      execucaoPorAutor[autor].valorExecutado += lancamentosAutor.reduce(
        (sum, l) => sum + (l.valor || 0),
        0,
      );
      execucaoPorAutor[autor].quantidadeLancamentos += lancamentosAutor.length;
    });
  ...
  const getExecucaoPorTipo = () => {
    ...
    emendas.forEach((emenda) => {
      ...
      const lancamentosTipo = lancamentos.filter(
        (l) => l.emendaId === emenda.id,
      );
      execucaoPorTipo[tipo].valorExecutado += lancamentosTipo.reduce(
        (sum, l) => sum + (l.valor || 0),
        0,
      );
    });
  ...
  const getFornecedores = () => {
    ...
    lancamentos.forEach((lancamento) => {
      const fornecedor = lancamento.notaFiscalFornecedor || "Não informado";
      ...
      fornecedores[fornecedor].valorTotal += lancamento.valor || 0;
      fornecedores[fornecedor].quantidadeNotas += 1;

      const dataLancamento = new Date(lancamento.data || 0);
      if (
        !fornecedores[fornecedor].ultimaCompra ||
        dataLancamento > new Date(fornecedores[fornecedor].ultimaCompra)
      ) {
        fornecedores[fornecedor].ultimaCompra = lancamento.data;
      }
    });
                ...
                <tbody>
                  {execucaoPorAutor.map((item, index) => (
                    ...
                    >
                      ...
                      <td style={styles.td}>{item.quantidadeLancamentos}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
